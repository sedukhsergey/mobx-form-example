export default class Users {
  ...
  async create(data) {
    ...
    const results = await this.databaseService.pool.query(sql, [data.name, data.email]);
    return results.rows[0];
  }

  async update(data) {
    const sql = 'UPDATE users SET name = $1, email = $2 WHERE id = $3';
    const results = await this.databaseService.pool.query(sql, [data.name, data.email, data.id]);
    return results.rows;
  }

  async delete(data) {
    const sql = 'DELETE FROM users WHERE id = $1';
    return await this.databaseService.pool.query(sql, [data]);
  }
}
